...
function generateJoiningInstructionsEmailHtml(
  companyName: string,
  coursesHtml: string,
  bookingCount: number
): string {
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <h2 style="color: #0f3d5e; margin-top: 30px;">Your Training Course${bookingCount > 1 ? 's' : ''}</h2>
          <p style="margin-bottom: 20px;">Below are the details for ${bookingCount > 1 ? 'all your upcoming courses' : 'your upcoming course'}:</p>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const emailHtml = generateJoiningInstructionsEmailHtml(companyName, coursesHtml, bookings.length);
