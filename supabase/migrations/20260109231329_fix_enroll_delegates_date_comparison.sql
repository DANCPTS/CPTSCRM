/*
  # Fix date comparison in enroll_delegates trigger

  1. Changes
    - Fix the date_of_birth comparison that was comparing a date type to an empty string
    - Remove invalid `!= ''` check for date columns
    - Only check for NULL since date columns cannot contain empty strings

  2. Notes
    - The previous trigger compared `date_of_birth != ''` which is invalid for date types
    - This caused "invalid input syntax for type date" errors when creating bookings
*/
