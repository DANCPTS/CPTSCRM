/*
  # Update auto-create candidates trigger to use delegate-specific contacts

  1. Changes
    - Update trigger to extract email and phone from delegate if provided
    - Fall back to booker's email/phone if delegate doesn't provide their own
    - This allows delegates to optionally provide their own contact information

  2. Behavior
    - If delegate provides email/phone, use those
    - Otherwise, use the booking contact's email/phone as before
*/
  ...
  IF NEW.status = 'signed' AND (OLD.status IS NULL OR OLD.status != 'signed') THEN
    ...
    IF NEW.form_data->'delegates' IS NOT NULL THEN
      ...
      LOOP
        ...
        -- Use delegate-specific email/phone if provided, otherwise fall back to booker's
